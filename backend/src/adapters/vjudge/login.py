"""VJudge 一键登录（browser-login）：Playwright 拉起系统浏览器，用户自行登录。

设计见 docs/design/activity/vjudge.md：
- 复用系统已装浏览器（channel="chrome" 兜底 "msedge"），不下载浏览器二进制；
- 临时独立 profile（launch() 默认临时用户目录）：不碰用户日常浏览器数据，
  登录态随窗口关闭即焚；
- 用户在真实浏览器里自行完成登录（Cloudflare Turnstile 等自然通过），
  应用轮询受控上下文的 cookie 罐：双 cookie（JSESSIONID + JSESSlONID）
  出现为候选信号，再经鉴权探针（/user/submissions 返回 data 数组）
  确认完整登录态才抓取；
...
logger = logging.getLogger("xcpc.adapters.vjudge.login")

# VJudge 登录入口页面（登录为模态框，需先打开含登录入口的页面）
LOGIN_ENTRY_URL = "https://vjudge.net/status"
COOKIE_URL = "https://vjudge.net"
# 鉴权探针端点：仅完整登录态可访问
AUTH_PROBE_URL = "https://vjudge.net/user/submissions"
# 登录成功判定所需的 cookie 名
REQUIRED_COOKIES = ("JSESSIONID", "JSESSlONID")
...
async def _session_authed(context, handle: str) -> bool:
    """鉴权探针：用浏览器上下文请求登录态接口，确认会话完整登录。

    /user/submissions 在未完整登录时返回 error.i18nKey="user.error.login_required"，
    完整登录才返回 data 数组。探针失败（网络波动等）按未登录处理——
    下一轮轮询重试，不误判成功。
    """
    try:
        resp = await context.request.get(
            AUTH_PROBE_URL,
            params={"username": handle, "pageSize": "1"},
            timeout=10_000,
        )
        if resp.status != 200:
            return False
        ...
        return False
    return isinstance(data, dict) and "data" in data and "error" not in data


async def capture_credentials(
    handle: str, timeout: float = DEFAULT_TIMEOUT
) -> Credentials:
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            while time.monotonic() < deadline:
                ...
                # 双 cookie 出现只是候选信号，必须再经鉴权探针确认完整登录态
                ...
                if cookies_ready and probe_due:
                    ...
                    if await _session_authed(context, handle):
                        return Credentials(
            ...
            raise PlatformError(f"打开 VJudge 页面失败: {exc}") from exc
