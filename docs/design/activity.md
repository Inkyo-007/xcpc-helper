...
统一存储在 `backend/data/user/<userid>/`，**每个用户组一个目录（目录名即组名，支持中文）**；服务层维护当前用户组（内存态，默认 `default` 惰性初始化），其余 API 一律作用于当前组。

非敏感数据中 `example/` 样例入 git（延续"git 管理数据目录"约定），用户组运行数据 gitignore：
...
**用户组**：存储层带 userid 维度（见 §3.1），前端用户组菜单展示当前用户组名（目录名），菜单顶部为「新建用户组」，下方为用户组列表，点击切换；重命名与删除在「编辑用户组」弹窗中操作（见 §4.2 工具条），至少保留一个组，删除当前组自动切换到剩余首个组。**组名 = data/user/<user_id> 目录名**：新建/重命名同步目录名（支持中文），删除为物理删除（账号绑定、训练数据、信息卡一并删除，前端明确提示不可找回）。各用户组的档案（ID / 签名 / 头像）、账号绑定与训练数据互相隔离；**信息栏 ID 与组名分离**：信息卡编辑 ID 只改 profile.json，重命名组不影响信息卡。

### 4.2 页面区块

页面主体为左右双栏（不出现分界线，仅以间距区分）：左栏较窄，右栏较宽；顶部工具条通栏。

1. **工具条**（通栏）：左侧平台分段切换器（汇总 + 全部支持平台）；右侧同步区——上次同步时间（等宽字体 muted）、「立即同步」按钮（lucide `RefreshCw`，同步中旋转禁用；同步范围随视图——**汇总视图同步全部平台，点击先弹确认框**说明会同步全部平台数据、可能较慢，**平台视图只同步该平台**，直接触发）、「编辑用户组」按钮（lucide `UserRoundPen`，**仅汇总视图出现**）、以及随视图切换形态的账号入口：
   - **编辑用户组弹窗**：列表式交互（每项一行，左描述右操作）——「重命名用户组」（输入框 + 保存，修改组名 = data/user 目录名，数据归属不变；信息卡 ID 独立不受影响）、「删除用户组」（DeleteConfirmModal 确认，删除该组全部数据，删当前组后弹窗关闭并回退）、「平台账号」列表（按平台逐行：已绑定的显示 handle，右侧「换绑」「解绑」（DeleteConfirmModal 确认，解绑删除该账号本地数据）；未绑定的右侧「绑定」，均打开锁定该平台的绑定弹窗；同步失败在描述中标注）；
   - **汇总视图**：用户组下拉菜单，按钮显示当前用户组名（见 §4.1「用户组」）；
...
3. **左栏 · 近期提交**：跨平台合并的最近提交，较新在上。每行：verdict 徽章、题目基本信息（题号 + 题名，点击跳平台原站外链）、平台与提交时间（**平台显示在行底**，取代此前的提交语言，为题名腾出空间）。**每页固定 10 条**，底部放分页导航（页码同步到网址，见 §4.7；页码栏至多 7 个槽位，形如 `1 2 3 4 5 … 末页`，槽位再宽会溢出左栏）。点击热力图格子后切换为当日明细（标题变为该日期），**当日明细同样每页 10 条分页**，两种模式的页码状态各自独立；不提供返回按钮——**再次点击同一格子取消选中**，即回到近期提交。
...
| 1 | 有提交、0 AC | 最低透明度档 |
| 2–5 | AC ≥ 1，按 AC 数分桶（1 / 2–3 / 4–5 / ≥6） | 透明度逐级升高 |
...
- **解绑**：「编辑用户组」弹窗的平台账号列表内操作，DeleteConfirmModal 同款确认；解绑删除该账号本地数据，明确提示不可找回；
- **同步遮罩**：首次绑定（含换绑）与手动同步期间弹出全屏加载遮罩——半透明模糊背景使页面不可操作，指示器为迷你热力格按对角线错峰点亮的波动动画（呼应热力图），结束后淡出。
...
| POST | `/api/activity/groups` | 新建用户组 `{name}`（目录创建 + 初始信息卡，自动切换，201） |
| PATCH | `/api/activity/groups/{name}` | 重命名用户组 `{newName}`（目录改名，数据归属不变） |
| DELETE | `/api/activity/groups/{name}` | 删除用户组（物理删除全部数据；当前组被删回退，至少保留一个组，204） |
| POST | `/api/activity/current-group` | 切换当前用户组 `{name}` |
| GET | `/api/activity/profile` | 当前组信息卡（ID / 签名 / 头像） |
| PATCH | `/api/activity/profile` | 更新当前组信息卡（ID 与组名分离，avatar 显式 null 清除） |
