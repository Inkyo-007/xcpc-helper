...
└─ <userid>/                    # 用户组（组名 = 目录名，可新建/重命名/删除）
   ...
   └─ secrets.json              # cookie 等凭据（gitignore，仅存本机；预留）
```
...
- 凭据平台（后续）：绑定弹窗录入 cookie，`verify`/同步携带 `credentials`；
  同步中 `AuthExpiredError` → `syncErrorCode: "auth_expired"` → 前端引导重新授权。
...
└─ modules/activity/
   ├─ models.py                # Submission / Account / Profile（信息卡+账号）/ SyncStatus 领域模型
   ├─ schemas.py               # API 出入参 DTO（camelCase，与前端 types.ts 对齐）
   ├─ store.py                 # data/user/<userid>/ 读写层 + 用户组目录管理（原子写、锁）
...
- **信息卡**：读写当前组 `profile.json`（ID 与组名分离，avatar 显式 null 清除、
  上限 500k 字符）；
- **账号**：绑定（换绑删旧）、解绑、验证（能力校验 + `credentials` 透传，
  `UserNotFoundError → 400`、`PlatformError → 502`）；
...
| GET | `/api/activity/platforms` | 平台元数据（id/名称/capabilities/auth）+ 已绑定账号及同步状态 |
| POST | `/api/activity/accounts/verify` | 校验 `{platform, handle, credentials?}`；用户不存在 400，平台故障 502 |
| POST | `/api/activity/accounts` | 绑定 `{platform, handle, credentials?}`（换绑删旧），自动触发首次同步（201） |
...
│  ├─ AccountBindModal.vue     # 绑定 / 换绑 / 凭据录入
