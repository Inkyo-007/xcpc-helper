...
**用户组**：存储层第一天带 userid 维度（见 §3.1），前端原型阶段即暴露用户组管理：工具条右侧的用户组下拉菜单展示当前用户 ID（与左侧用户信息卡的主标签一致），菜单顶部为「新建用户组」，下方为用户组列表，点击切换；重命名与删除在「编辑用户组」弹窗中操作（见 §4.2 工具条），至少保留一个组，删除当前组自动切换到剩余首个组。各用户组的档案（ID / 签名 / 头像）、账号绑定与训练数据互相隔离；用户信息卡编辑 ID 即重命名当前组（数据归属不变，靠内部稳定 key 区分）。第一期实现中训练数据尚未按组隔离，删除用户组只删除其档案（ID / 签名 / 头像）。
...
1. **工具条**（通栏）：左侧平台分段切换器（汇总 + 全部支持平台）；右侧同步区——上次同步时间（等宽字体 muted）、「立即同步」按钮（lucide `RefreshCw`，同步中旋转禁用；同步范围随视图——**汇总视图同步全部平台，点击先弹确认框**说明会同步全部平台数据、可能较慢，**平台视图只同步该平台**，直接触发）、「编辑用户组」按钮（lucide `UserRoundPen`，**仅汇总视图出现**）、以及随视图切换形态的账号入口：
   - **编辑用户组弹窗**：列表式交互（每项一行，左描述右操作）——「重命名用户组」（输入框 + 保存，等价于用户信息卡编辑 ID）、「删除用户组」（DeleteConfirmModal 确认，删当前组后弹窗关闭）、「平台账号」列表（按平台逐行：已绑定的显示 handle，右侧「换绑」「解绑」（DeleteConfirmModal 确认，解绑删除该账号本地数据）；未绑定的右侧「绑定」，均打开锁定该平台的绑定弹窗；同步失败在描述中标注）；
