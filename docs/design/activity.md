...
1. **工具条**（通栏）：左侧平台分段切换器（汇总 + 全部支持平台）；右侧同步区——上次同步时间（等宽字体 muted）、「立即同步」按钮（lucide `RefreshCw`，同步中旋转禁用；同步范围随视图——**汇总视图同步全部平台，点击先弹确认框**说明会同步全部平台数据、可能较慢，**平台视图只同步该平台**，直接触发）、「编辑用户组」按钮（lucide `UserRoundPen`，**仅汇总视图出现**）、以及随视图切换形态的账号入口：
   - **编辑用户组弹窗**：列表式交互（每项一行，左描述右操作）——「重命名用户组」（输入框 + 保存，等价于用户信息卡编辑 ID）、「删除用户组」（DeleteConfirmModal 确认，删当前组后弹窗关闭）、「已绑定账号」列表（每账号一行，右侧「换绑」按钮打开锁定该平台的绑定弹窗；同步失败在描述中标注）。解绑入口随账号管理弹层一并移除（后端与 store 能力保留）；
...
- **解绑**：随账号管理弹层移除，前端暂不提供解绑入口（后端接口与 store 动作保留）；需要清除账号数据时通过换绑覆盖；
