...
- **PDF 生成管线**：打印视图（静态 HTML）→ Paged.js 在浏览器内分页（自动页脚页码、目录页码由 `target-counter` 回填）→ 浏览器打印"另存为 PDF"；桌面模式（pywebview）后续可免费升级为 printToPDF 一键导出。
...
- **块存引用不存内容**：template 块只记录模板 id 与版本，预览/导出时实时从模板索引取最新代码，模板更新自动反映；已打印的 PDF 是快照，封面与页脚带生成日期。
...
- **heading_level**：取值 1-6；heading 块默认 2，template 块默认 3；用户添加块时若手动改过级别，前端按块类型记忆该选择，下次添加同类型块时沿用。
...
3. **模板库紧凑列表**：带搜索、排序、分类筛选（与模板库同源数据）；模板可展开到版本粒度，**点击某个版本**即以该版本插入 template 块。

**插入位置**：所有添加动作默认追加到条目尾部；同时提供位置选项，可指定"添加在第 N 个条目之后"，避免长列表下"先加尾部再拖拽"的往返。
...
- **屏幕预览中的目录不显示页码**（未分页无法得知）；页码在导出时由 Paged.js 填入真实页码，浏览器打印预览中即为成品。即"所见即所得"精确到打印预览那一步，屏幕预览是近似布局。
- 顶部"导出 PDF"按钮：切换/复制为打印视图 DOM（打印样式），Paged.js 分页（页脚页码 + `target-counter` 目录页码）后唤起浏览器打印，文件名取封面标题。
