...
> 本文档面向希望参与开发或从源码部署的开发者。
> 普通用户请阅读根目录 [README.md](../README.md)。
...
洛谷等 cookie 平台的「一键登录」需要可选依赖组 `browser-login`（Playwright，复用系统
...
| 后端静态检查 | `uv run ruff check src tests` | `backend/` |
